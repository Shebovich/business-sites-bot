...
// чуть разное (#171, сотни комбинаций → анти-спам #150). Человеческий тон, всегда
// «Добрый день», без продаванских штампов (#169), без упоминания товара (#154)/рекламы (#155).
const pick = (a) => a[Math.floor(Math.random() * a.length)];
function outreachMessage() {
  const who = pick([
    'Я делаю сайты.', 'Занимаюсь сайтами для бизнеса.', 'Я веб-разработчик.',
    'Делаю сайты для небольших компаний.', 'Я создаю сайты.', 'Делаю сайты под ключ.',
  ]);
  // Бесплатен только ПРОТОТИП/пробный вариант (не весь сайт), #173.
  const offer = pick([
    'Хочу предложить — могу за пару дней бесплатно сделать прототип вашего сайта, просто чтобы показать.',
    'Готов бесплатно собрать пробный вариант сайта за пару дней — для примера.',
    'Могу за пару дней бесплатно набросать пробный макет сайта.',
    'Хочу бесплатно сделать прототип сайта за пару дней и прислать посмотреть.',
    'Подумал, что вам пригодится — сделаю бесплатно пробный вариант сайта за пару дней.',
  ]);
  const calm = pick([
    'Понравится — доработаем под вас и обсудим стоимость, нет — ничего страшного.',
    'Ни к чему не обязывает — просто посмотрите готовый пример.',
    'Если зайдёт — обсудим детали и цену, нет — без проблем.',
    'Без обязательств, просто покажу, что может получиться.',
    'Посмотрите спокойно и решите, делать ли дальше.',
  ]);
  const cta = pick(['Покажу?', 'Интересно глянуть?', 'Что скажете?', 'Скинуть посмотреть?', 'Подойдёт?', '']);
  return `Добрый день! ${who} ${offer} ${calm}${cta ? ' ' + cta : ''}`;
}
