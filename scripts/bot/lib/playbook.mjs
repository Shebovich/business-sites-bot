...
<b>2. Собрать фото (опционально)</b>
Никакая секция не обязательна. Шли что хочешь — owner решит на ревью.
...
<b>2a. Общая заметка (без секции)</b>
• Закрой текущую секцию (/cancel или просто не выбирай) → пиши текст
• /note {текст} — сохранит как общую заметку к задаче
• Полезно если хочешь дать owner'у общий контекст ("всё устраивает, отправляю как есть", "shoot фото у меня нет, делай по IG")

<b>3. Проверить перед отправкой</b>
• /preview → увидишь свою же сводку: фото-счётчики, тексты, заметки
• Хочешь удалить — /rm {section} {N}
• Хочешь отменить skip — /unskip {section}

<b>4. Передать на ревью</b>
• /submit — в любой момент, даже если ничего не собрал
• Owner получит push с твоей сводкой и решит сам
• Если ничего не поменял после feedback'а — /submit отклонит дубликат
